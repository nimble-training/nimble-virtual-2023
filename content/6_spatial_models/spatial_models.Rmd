...
MCMC <- buildMCMC(conf)
cMCMC <- compileNimble(MCMC, project = cModel)
system.time(output <- runMCMC(cMCMC, niter = 10000, nburnin = 5000, thin = 5, WAIC = TRUE))
samples <- output$samples
waic_gp <- output$WAIC
```
  ...
  run = function(dists = double(2), rho = double(0), sigma = double(0)) {
    ...
    for(i in 1:n)
      for(j in 1:n)
        result[i, j] <- sigma2*exp(-dists[i,j]/rho)
    return(result)
  }, buildDerivs = list(run = list(ignore = c('i','j','dists'))))
...
The posteriors from HMC seem to be somewhat different from the Metropolis-based sampling. We'll need to look into this to better understand what is happening.  
