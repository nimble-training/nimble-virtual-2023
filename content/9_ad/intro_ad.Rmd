...
* Huge credit to [<span style="color:blue">CppAD</span>](https://coin-or.github.io/CppAD/html/CppAD.html).
* Written by Brad Bell at the University of Washington.
* CppAD is also used by Template Model Builder (TMB).
* CppAD is general and efficient.
...
* Define $n$ = number of inputs and $m$ = number of outputs.
...
* For calculations outside of a model: `nimDerivs(foo(x), ...)` , i.e. "nimDerivs of foo".
...
* random effect: $x$
* parameters: $\theta$
* Need: $\int P(y | x, \theta) P(x | \theta) dx$.
* Approximate by:

  * Find $x^{*}$ that maximizes $P(y | x, \theta) P(x | \theta)$.
...
# Fourth exampe: Laplace approximation
...
* We can consider various ways to arrange the different derivatives using double-taping schemes.
...
This slide shows use of the fully built Laplace approximation to check above demostration code.  We don't need to go through it.
...
* First parameter is the Sex1 intercept for both.
* Second parameter from `nimble` is the Sex2 intercept, so it is `(Intercept)` + `fSex2` from `lme4`.
* Third parameter from `nimble` is the Sex1 coefficient for `ctrLength`.
* Fourth parameter from `nimble` is the Sex2 coefficient for `ctrLength`, so it is `ctrLength` + `ctrLength:fSex2` from `lme4`.
