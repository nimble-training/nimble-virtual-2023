...
The original code did add the different terms and have mean zero random effects, so it's already using a partially non-centered parameterization. But it didn't pull out the `farm_sd` parameter.
...
model <- nimbleModel(DEcode_hmc, constants = DEconstants, inits = DEinits_vals, data = DEdata, buildDerivs = TRUE)
cmodel <- compileNimble(model)
...
### Call `compareMCMCs` for HMC (fewer samples) 


```{r}
params <- model$getNodeNames(topOnly = TRUE)
params <- params[!params %in% model$expandNodeNames('farm_effect')]
...
Results that come with these slides are [here for uncentered length covariate](orig_deer_ecervi_uncLen_results.html) and [here for the centered random effects (and centered length covariate) plus HMC](orig_deer_ecervi_ctrRE_results.html).

Results if you run the code yourself will be [here for uncentered length covariate](deer_ecervi_uncLen_results.html) and [here for the centered random effects (and centered length covariate) plus HMC](deer_ecervi_ctrRE_results.html).
